...
const Project = () => {
  ...
  return (
    ...
    //   {/* LEFT PANEL */}
    //   <section className="relative flex flex-col h-[50vh] lg:h-screen min-h-screen  bg-indigo-100 w-full ">
    //     {/* HEADER */}
    ...
    //     {/* MESSAGES */}
    //     <div className=" lg:mt-0 mt-16 pb-14 flex-grow flex flex-col relative overflow-hidden ">
    //       <div
    //         ref={messageBox}
    //         className="flex-grow p-4 flex flex-col gap-3 overflow-y-auto scrollbar-hide"
    //       >
    ...
    //             className={`${
    //               msg.sender._id === "ai"
    //                 ? "bg-violet-100 max-w-80"
    //                 : "bg-white max-w-52"
    //             } ${msg.sender._id == user._id.toString() && "ml-auto"}
    //           message flex flex-col px-4 py-2 w-fit shadow-md rounded-xl`}
    //           >
    ...
    //       {/* INPUT */}
    //       <div className="absolute bottom-0 left-0 w-full bg-indigo-200 flex p-2 gap-2">
    //         <input
    //           value={message}
    ...
    //         <button
    //           onClick={send}
    //           className="bg-indigo-700 hover:bg-indigo-800 text-white px-4  rounded-md transition"
    //         >
    ...
    //   {/* RIGHT PANEL — YOU CAN PLACE YOUR CODE EDITOR OR ANYTHING HERE */}
    ...
    // <main className="min-h-screen w-screen  flex  font-sans overflow-hidden">
    //   {/* LEFT PANEL */}
    //   <section className="relative flex flex-col h-screen lg:h-screen lg:min-w-96 bg-indigo-100 w-full max-w-full overflow-hidden">
    //     {/* HEADER */}
    //     <header className="flex justify-between items-center p-2 sm:p-3 px-4 sm:px-5 bg-indigo-300 shadow-md z-10 fixed top-0 w-full lg:static">
    //       <button
    //         className="flex items-center gap-1 sm:gap-2 text-indigo-800 font-semibold hover:text-indigo-900 text-sm transition"
    //         onClick={() => setIsModalOpen(true)}
    //       >
    //         <i className="ri-add-fill text-lg sm:text-xl"></i>
    //         <span className="hidden xs:inline">Add</span>
    //         <span className="hidden sm:inline">Collaborator</span>
    //       </button>
    //       <button
    //         onClick={() => setIsSidePanelOpen(!isSidePanelOpen)}
    //         className="text-indigo-900 hover:text-indigo-950 p-1 sm:p-2"
    //       >
    //         <i className="ri-group-fill text-lg sm:text-xl"></i>
    //       </button>
    //     </header>

    //     {/* MESSAGES */}
    //     <div className="  pb-14 flex-grow flex flex-col overflow-hidden">
    //       <div
    //         ref={messageBox}
    //         className="flex-grow px-2 sm:px-4 py-2 flex flex-col gap-3 overflow-y-auto"
    //       >
    //         {messages.map((msg, index) => (
    //           <div
    //             key={index}
    ...
    //                 : "bg-white max-w-[70%]"
    //             } ${msg.sender._id == user._id.toString() && "ml-auto"}
    //         message flex flex-col px-3 py-2 w-fit shadow-md rounded-xl text-sm`}
    //           >
    //             <small className="block text-[10px] text-gray-500 mb-1 truncate">
    //               {msg.sender.email}
    //             </small>
    ...
    //       {/* INPUT */}
    //       <div className="absolute bottom-0 left-0 w-full bg-indigo-200 flex p-1 sm:p-2 gap-1 sm:gap-2 items-center">
    //         <input
    //           value={message}
    //           onChange={(e) => setMessage(e.target.value)}
    //           className="flex-grow px-2 py-1 sm:py-2 rounded-md border-none outline-none text-sm bg-white shadow-inner"
    //           type="text"
    //           placeholder="Message"
    //         />
    //         <button
    //           onClick={send}
    //           className="bg-indigo-700 hover:bg-indigo-800 text-white px-3 sm:px-4 py-1 rounded-md transition"
    //         >
    ...
    //         </button>
    //       </header>
    //       <div className="users p-4 space-y-3 overflow-y-auto">
    //         {project.users?.map((user) => (
    //           <div
    //             key={user._id}
    //             className="flex items-center gap-3 p-3 hover:bg-indigo-100 rounded-md cursor-pointer"
    //           >
    //             <div className="bg-indigo-500 text-white w-9 h-9 flex items-center justify-center rounded-full text-sm">
    //               <i className="ri-user-fill"></i>
    //             </div>
    //             <span className="text-indigo-800 text-sm truncate">
    //               {user.email}
    //             </span>
    //           </div>
    //         ))}
    ...
    //       <div className="bg-white p-3 sm:p-6 rounded-lg shadow-lg w-full max-w-xs sm:max-w-md relative">
    //         <header className="flex justify-between items-center mb-4">
    //           <h2 className="text-sm sm:text-lg font-bold text-indigo-800">
    //             Select Users
    //           </h2>
    //           <button onClick={() => setIsModalOpen(false)}>
    //             <i className="ri-close-fill text-xl text-gray-500 hover:text-red-500"></i>
    //           </button>
    //         </header>
    //         <div className="users-list space-y-2 max-h-64 overflow-y-auto mb-4">
    //           {users
    //             .filter(
    //               (u) =>
    //                 !project.users.some((pUser) => pUser._id === u._id) &&
    ...
    //                 onClick={() => handleUserClick(filteredUser._id)}
    //               >
    //                 <div className="bg-indigo-600 text-white w-9 h-9 flex items-center justify-center rounded-full text-sm">
    //                   <i className="ri-user-fill"></i>
    //                 </div>
    //                 <span className="text-sm">{filteredUser.email}</span>
    //               </div>
    //             ))}
    //         </div>
    //         <button
    //           onClick={addCollaborators}
    //           className="w-full py-2 bg-indigo-700 text-white rounded-md hover:bg-indigo-800 transition text-sm"
    //         >
    //           Add Collaborators
    //         </button>
    //       </div>
